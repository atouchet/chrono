...
impl NaiveTime {
    ...
    ///
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 900)),
    ///            TimeDelta::Forwards(Duration::from_secs(0)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 875)),
    ///            TimeDelta::Forwards(Duration::from_millis(25)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 6, 925)),
    ///            TimeDelta::Forwards(Duration::from_millis(975)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 0, 900)),
    ///            TimeDelta::Forwards(Duration::from_secs(7)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 0, 7, 900)),
    ///            TimeDelta::Forwards(Duration::from_secs(5 * 60)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(0, 5, 7, 900)),
    ///            TimeDelta::Forwards(Duration::from_secs(3 * 3600)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(4, 5, 7, 900)),
    ///            TimeDelta::Backwards(Duration::from_secs(3600)));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(2, 4, 6, 800)),
    ///            TimeDelta::Forwards(Duration::from_secs(3600 + 60 + 1) + Duration::from_millis(100)));
    /// ```
    ///
    /// Leap seconds are handled, but the subtraction assumes that
    /// there were no other leap seconds happened.
    ...
    /// assert_eq!(since(from_hmsm(3, 0, 59, 1_500), from_hmsm(3, 0, 59, 0)),
    ///            TimeDelta::Forwards(Duration::from_millis(1500)));
    /// assert_eq!(since(from_hmsm(3, 0, 59, 1_000), from_hmsm(3, 0, 0, 0)),
    ///            TimeDelta::Forwards(Duration::from_secs(60)));
    /// assert_eq!(since(from_hmsm(3, 0, 0, 0), from_hmsm(2, 59, 59, 1_000)),
    ///            TimeDelta::Forwards(Duration::from_secs(1)));
    /// assert_eq!(since(from_hmsm(3, 0, 59, 1_000), from_hmsm(2, 59, 59, 1_000)),
    ///            TimeDelta::Forwards(Duration::from_secs(61)));
    /// ```
    pub fn duration_since(self, rhs: NaiveTime) -> TimeDelta {
        //     |    |    :leap|    |    |    |    |    |    |    :leap|    |
        //     |    |    :    |    |    |    |    |    |    |    :    |    |
        // ----+----+-----*---+----+----+----+----+----+----+-------*-+----+----
        //          |   `rhs` |                             |    `self`
        ...
        match self.secs.cmp(&rhs.secs) {
            ...
            Ordering::Equal if self.frac >= rhs.frac + SECOND_AS_NANOS => {
                TimeDelta::Forwards(Duration::new(1, self.frac - rhs.frac - SECOND_AS_NANOS))
            }
            // self.frac may be leap but resulting frac is not
            Ordering::Equal if self.frac >= rhs.frac => {
                TimeDelta::Forwards(Duration::new(0, self.frac - rhs.frac))
            }
            ...
            Ordering::Equal if rhs.frac >= self.frac + SECOND_AS_NANOS => {
                TimeDelta::Backwards(Duration::new(1, rhs.frac - self.frac - SECOND_AS_NANOS))
            }
            // rhs is larger but the difference is less than a
            Ordering::Equal => TimeDelta::Backwards(Duration::new(0, rhs.frac - self.frac)),
            Ordering::Greater => {
                // must be `Forwards`
                ...
                TimeDelta::Forwards(Duration::new(secs.into(), frac))
            }
            Ordering::Less => {
                // must be `Backwards`
                ...
                TimeDelta::Backwards(Duration::new(secs.into(), frac))
            }
        }
    }
